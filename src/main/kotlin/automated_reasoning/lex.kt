 ...
 */
tailrec fun lex(inp: String, tokens: MutableList<String> = mutableListOf()): List<String> {
    val inpMinusSpaces = lexWhile(inp = inp, prop = space).second
    return when {
        inpMinusSpaces.isEmpty() -> tokens.toList()
        else -> {
...
/**
 * Here's a possible alternative to the lexWhile() fun
 * see also public inline fun CharSequence.dropWhile(predicate: (Char) -> Boolean): CharSequence
 */
private inline fun String.myDropWhile(predicate: (Char) -> Boolean): Pair<String, String> {
    for (index in indices) if(!predicate(this[index])) return Pair(substring(0 until index), substring(index))
    return Pair(this, "")
}
