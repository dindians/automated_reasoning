internal tailrec fun parseExpressionTailrec(tokens: List<String>, makeExpression: (Expression) -> Expression = { it }): Pair<Expression, List<String>> {
    val (expression, unparsedTokens) = parseProduct(tokens, makeExpression)
    return when {
...
private tailrec fun parseProduct(tokens: List<String>, makeExpression: (Expression) -> Expression): Pair<Expression, List<String>> {
    val (expression, unparsedTokens) = parseAtom(tokens)
    return when {
        unparsedTokens.isEmpty() || unparsedTokens.first() != "*" -> Pair(makeExpression(expression), unparsedTokens)
        else -> parseProduct(unparsedTokens.drop(1)) { makeExpression(Expression.Mul(expression, it)) }
    }
}
...
private fun parseAtom(tokens: List<String>): Pair<Expression, List<String>> {
    ...
    return when (firstToken) {
        "(" -> {
            val (expression, unparsedTokens) = parseExpressionTailrec(remainingTokens) { it }
